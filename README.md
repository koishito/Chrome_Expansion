...
#### background.js
- アクティブタブ遷移時に実行するには：chrome.runtime.onStartup.addListenerに登録
...
- manifest.jsonのcontent_scripts.run_atプロパティで読み込む順番を少しだけ制御できます。
  - document_start
  - document_end
...
- chrome.tabs.onActivated.addListenerにて、アクティブタブ切り替え毎に実行するJavaScriptの登録が可能。
